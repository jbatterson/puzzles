...
function loadCompletions(dateKey) {
  return [0, 1, 2].map((i) =>
    ['1', '2'].includes(localStorage.getItem(`factorfall:${dateKey}:${i}`))
  )
}
...
function loadGameState(dateKey, puzzleIndex, puzzle) {
  try {
    ...
    if (
      ...
      !d.queue.every((b) => typeof b?.v === 'number' && typeof b?.c === 'string')
    )
      return null
    if (d.gameState !== 'READY' && d.gameState !== 'GAMEOVER') return null
    if (
      !d.ui ||
      typeof d.ui.boardCleared !== 'boolean' ||
      typeof d.ui.gridFull !== 'boolean' ||
      typeof d.ui.outOfBalls !== 'boolean'
    )
      return null
...
const Factorfall = () => {
  ...
  const [mode, setMode] = useState(
    () => getInitialTutorialNav(GAME_KEYS.FACTORFALL, puzzleData.tutorial ?? []).mode
  )
  const [tutorialIdx, setTutorialIdx] = useState(
    () => getInitialTutorialNav(GAME_KEYS.FACTORFALL, puzzleData.tutorial ?? []).tutorialIdx
  )
  ...
  const tierSlots = useMemo(() => {
    ...
    return getEnabledTierIndices(GAME_KEYS.FACTORFALL)
  }, [suitePrefsEpoch])
  ...
  useSuiteCompletionTimer(GAME_KEYS.FACTORFALL, daily.key, {
    ...
    alreadyFullyComplete: isSuiteCompleteForPrefs(GAME_KEYS.FACTORFALL, daily.key),
  })
  ...
  useEffect(() => {
    if (curateMode || mode !== 'daily') return
    const c = clampDailyIndexToTierPrefs(GAME_KEYS.FACTORFALL, dailyIdx)
  ...
  useEffect(() => {
    const measure = () => {
      ...
      const newCell = Math.min(Math.floor(w / COLS), Math.floor(h / (ROWS + 1)), BASE_CELL)
      setCellSize(newCell)
  ...
  function persistDaily(overrideUi) {
    ...
    if (cm) {
      ...
      saveGameState(
        ...
        overrideUi ?? uiSnapshotRef.current
      )
      return
    }
    if (m !== 'daily') return
      ...
      for (let r = 0; r < gs.grid[c].length; r++) {
        ...
        const findSets = (currentKeys, currentProd, neighbors) => {
          if (currentProd === target) {
            ...
            if (!foundSetIDs.has(id)) {
              foundSetIDs.add(id)
              gs.highlightQueue.push([...currentKeys])
            }
            return
          }
            ...
            if (target % nextProd === 0) {
              ...
              ;[
                ...
                if (
                  ...
                  nnC < COLS &&
                  nnR >= 0 &&
                  nnR < gs.grid[nnC].length &&
                  gs.grid[nnC][nnR].color === startBall.color &&
                  !nextKeys.includes(nnK) &&
                  !nextNeighbors.includes(nnK)
                )
        ...
        ;[
          ...
          if (
            ...
            nc < COLS &&
            nr >= 0 &&
            nr < gs.grid[nc].length &&
            gs.grid[nc][nr].color === startBall.color
          )
            initN.push(`${nc},${nr}`)
        })
    ...
    if (boardCleared) {
      ...
      if (mode === 'tutorial') {
        if (tutorialIdx < puzzleData.tutorial.length - 1) setTutorialIdx((i) => i + 1)
          ...
          setMode('daily')
          setDailyIdx(clampDailyIndexToTierPrefs(GAME_KEYS.FACTORFALL, 0))
        }
      } else if (mode === 'daily') {
  ...
  const primaryLabel = useMemo(() => {
    ...
    if (boardCleared) {
      ...
      if (mode === 'tutorial') {
        return tutorialIdx < puzzleData.tutorial.length - 1
          ? CTA_LABELS.NEXT_PUZZLE
          : CTA_LABELS.PLAY_TODAY
      }
      if (mode === 'daily') {
        return nextIncompleteEnabledTierExcluding(GAME_KEYS.FACTORFALL, daily.key, dailyIdx) != null
          ? CTA_LABELS.NEXT_PUZZLE
  ...
  useEffect(() => {
    if (curateMode || mode !== 'daily') return
    const done = isSuiteCompleteForPrefs(GAME_KEYS.FACTORFALL, daily.key)
  ...
  useEffect(() => {
    ...
    const tid = window.setTimeout(
      () => setFactorfallPostWinCtaAttention(true),
      FACTORFALL_SUITE_MODAL_AFTER_WIN_MS
    )
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 1; i < COLS; i++) {
        ctx.beginPath()
        ctx.moveTo(i * cs, gridTop)
        ctx.lineTo(i * cs, canvasH)
        ctx.stroke()
      }
      for (let i = 2; i <= ROWS; i++) {
        ctx.beginPath()
        ctx.moveTo(0, i * cs)
        ctx.lineTo(canvasW, i * cs)
        ctx.stroke()
      }
      ...
      if (gs.gameState === 'READY' && gs.pendingBall)
        drawBall(gs.pendingBall.x, gs.pendingBall.y, gs.pendingBall.val, gs.pendingBall.color)
  ...
  return (
    <div className="game-container">
      ...
      <div className="button-tray">
        <button className="btn-secondary" onClick={undoMove} disabled={undoDisabled}>
          Undo
        </button>
        <button className="btn-secondary" onClick={resetLevel} disabled={undoDisabled}>
          Reset
        </button>
      </div>
      ...
      >
        ...
        <div style={{ flex: 1, textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '1.1rem', lineHeight: '1.6' }}>
            Drop factors into the grid. Groups of the same color that <b>multiply</b> to the target
