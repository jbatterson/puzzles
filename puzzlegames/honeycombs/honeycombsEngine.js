...
export function createHoneycombsEngine({
  ...
  function currentPuzzle() {
    return enginePuzzles[state.puzzleIdx]
  }
  ...
  function applyDigitToCell(row, col, num) {
    ...
    if (state.cells.some((c) => c.value === num && c.isClue && (c.row !== row || c.col !== col))) {
      return
    }
    pushMoveHistory()
    cell.value = num
...
`
  ...
  function renderKeyboard() {
    ...
    const playerUsed = new Set(
      state.cells.filter((c) => !c.isClue && c.value !== null).map((c) => c.value)
    )
