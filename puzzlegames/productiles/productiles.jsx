...
export default function Productiles() {
    ...
    const [dailyIdx, setDailyIdx]       = useState(() =>
        resolveHubDailySlotWithPrefs(GAME_KEYS.PRODUCTILES, getDailyKey(), typeof window !== 'undefined' ? window.location.search : ''),
    )
    ...
    const handlePrimary = () => {
        ...
        if (mode === 'tutorial') {
            ...
            else { setMode('daily'); setDailyIdx(clampDailyIndexToTierPrefs(GAME_KEYS.PRODUCTILES, 0)) }
        } else {
            ...
            {curateMode ? (
                ...
                <div className="level-nav">
                    <div className="left-spacer">
                        <button className="skip-link" onClick={() => { setMode('daily'); setDailyIdx(clampDailyIndexToTierPrefs(GAME_KEYS.PRODUCTILES, 0)) }}>
                            Skip Tutorial
                        </button>
                    </div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                    {!hasSeenInstructions ? (
                        <>
                            ...
                            <button className="btn-secondary" onClick={() => { closeInstructions(); setMode('daily'); setDailyIdx(clampDailyIndexToTierPrefs(GAME_KEYS.PRODUCTILES, 0)) }}>{CTA_LABELS.SKIP_TUTORIAL}</button>
                        </>
                    ) : (
                        <>
                            <button className="btn-primary" onClick={() => { closeInstructions(); setMode('daily'); setDailyIdx(clampDailyIndexToTierPrefs(GAME_KEYS.PRODUCTILES, 0)) }}>{CTA_LABELS.PLAY_TODAY}</button>
