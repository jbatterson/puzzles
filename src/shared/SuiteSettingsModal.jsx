...
export default function SuiteSettingsModal({ show, onClose, games, onSaved }) {
	...
	return (
		<FloatingModalShell show={show} onClose={onClose} intent={MODAL_INTENTS.SETTINGS} contentClassName="suite-settings-shell">
			<h2
			...
			<div style={{ marginBottom: '18px' }}>
				...
				<p style={{ margin: 0, fontSize: '0.95rem', lineHeight: 1.45, color: 'var(--puzzle-ink-soft, #4a5f72)' }}>
					Choose which puzzles appear on your dashboard.
				</p>
			</div>
			<div style={{ maxHeight: 'min(60vh, 420px)', overflowY: 'auto', margin: '0 -4px', padding: '0 4px' }}>
				{games.map(({ key, title }) => {
					...
					return (
						<div key={key} style={rowStyle}>
							...
							<div style={{ flex: 1, minWidth: 0, fontWeight: 800, fontSize: '0.95rem', color: PUZZLE_SUITE_INK }}>
								{title}
							</div>
							{tiers ? (
